...
# $ gen-test -T gen-compact-expr-bash-depth-2-base0-array
#
...
diff -u$u -L gen-compact-expr-bash-depth-2-base0-array.old <(echo \
'$ print() { printf '\''%s\n'\'' "$@"; }
$ printc() { awk '\''{ printf("%c\n", strtonum("0x" $1)) }'\'' <<< "$1"; }
...
) -L gen-compact-expr-bash-depth-2-base0-array.new <(
echo '$ print() { printf '\''%s\n'\'' "$@"; }'
print() { printf '%s\n' "$@"; } 2>&1 ||
...
echo '$ trie() { ../src/trie --gen=compact --output=expr --expr=bash --depth=2 --trie=array "$@"; }'
trie() { ../src/trie --gen=compact --output=expr --expr=bash --depth=2 --trie=array "$@"; } 2>&1 ||
echo 'command failed: trie() { ../src/trie --gen=compact --output=expr --expr=bash --depth=2 --trie=array "$@"; }'
