...
diff -u$u -L gen-wide-trie-base.old <(echo \
'$ print() { printf '\''%s\n'\'' "$@"; }
$ printc() { awk '\''{ printf("%c\n", strtonum("0x" $1)) }'\'' <<< "$1"; }
...
echo '$ trie() { ../src/trie --gen=wide --output=trie --attrs --no-dots "$@"; }'
trie() { ../src/trie --gen=wide --output=trie --attrs --no-dots "$@"; } 2>&1 ||
echo 'command failed: trie() { ../src/trie --gen=wide --output=trie --attrs --no-dots "$@"; }'
