...
# $ gen-test -T gen-wide-c-code-base-ternary
#
...
diff -u$u -L gen-wide-c-code-base-ternary.old <(echo \
'$ print() { printf '\''%s\n'\'' "$@"; }
$ printc() { awk '\''{ printf("%c\n", strtonum("0x" $1)) }'\'' <<< "$1"; }
...
.   }
.   return error;'
) -L gen-wide-c-code-base-ternary.new <(
echo '$ print() { printf '\''%s\n'\'' "$@"; }'
print() { printf '%s\n' "$@"; } 2>&1 ||
...
echo '$ trie() { ../src/trie --gen=wide --output=c-code --attrs --dots --trie=ternary "$@"; }'
trie() { ../src/trie --gen=wide --output=c-code --attrs --dots --trie=ternary "$@"; } 2>&1 ||
echo 'command failed: trie() { ../src/trie --gen=wide --output=c-code --attrs --dots --trie=ternary "$@"; }'
