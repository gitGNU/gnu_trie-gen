...
# $ gen-test -T gen-wide-expr-bash-depth-2-base-ternary
#
...
diff -u$u -L gen-wide-expr-bash-depth-2-base-ternary.old <(echo \
'$ print() { printf '\''%s\n'\'' "$@"; }
$ printc() { awk '\''{ printf("%c\n", strtonum("0x" $1)) }'\'' <<< "$1"; }
...
) -L gen-wide-expr-bash-depth-2-base-ternary.new <(
echo '$ print() { printf '\''%s\n'\'' "$@"; }'
print() { printf '%s\n' "$@"; } 2>&1 ||
...
echo '$ trie() { ../src/trie --gen=wide --output=expr --expr=bash --depth=2 --dots --trie=ternary "$@"; }'
trie() { ../src/trie --gen=wide --output=expr --expr=bash --depth=2 --dots --trie=ternary "$@"; } 2>&1 ||
echo 'command failed: trie() { ../src/trie --gen=wide --output=expr --expr=bash --depth=2 --dots --trie=ternary "$@"; }'
