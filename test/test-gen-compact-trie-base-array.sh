...
# $ gen-test -T gen-compact-trie-base-array
#
...
diff -u$u -L gen-compact-trie-base-array.old <(echo \
'$ print() { printf '\''%s\n'\'' "$@"; }
$ printc() { awk '\''{ printf("%c\n", strtonum("0x" $1)) }'\'' <<< "$1"; }
...
) -L gen-compact-trie-base-array.new <(
echo '$ print() { printf '\''%s\n'\'' "$@"; }'
print() { printf '%s\n' "$@"; } 2>&1 ||
...
echo '$ trie() { ../src/trie --gen=compact --output=trie --attrs --dots --trie=array "$@"; }'
trie() { ../src/trie --gen=compact --output=trie --attrs --dots --trie=array "$@"; } 2>&1 ||
echo 'command failed: trie() { ../src/trie --gen=compact --output=trie --attrs --dots --trie=array "$@"; }'
