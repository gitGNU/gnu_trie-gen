...
# $ gen-test -T gen-compact-expr-bash-depth-0-base-array
#
...
diff -u$u -L gen-compact-expr-bash-depth-0-base-array.old <(echo \
'$ print() { printf '\''%s\n'\'' "$@"; }
$ printc() { awk '\''{ printf("%c\n", strtonum("0x" $1)) }'\'' <<< "$1"; }
...
) -L gen-compact-expr-bash-depth-0-base-array.new <(
echo '$ print() { printf '\''%s\n'\'' "$@"; }'
print() { printf '%s\n' "$@"; } 2>&1 ||
...
echo '$ trie() { ../src/trie --gen=compact --output=expr --expr=bash --depth=0 --dots --trie=array "$@"; }'
trie() { ../src/trie --gen=compact --output=expr --expr=bash --depth=0 --dots --trie=array "$@"; } 2>&1 ||
echo 'command failed: trie() { ../src/trie --gen=compact --output=expr --expr=bash --depth=0 --dots --trie=array "$@"; }'
