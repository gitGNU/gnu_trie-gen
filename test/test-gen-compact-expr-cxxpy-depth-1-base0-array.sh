...
# $ gen-test -T gen-compact-expr-cxxpy-depth-1-base0-array
#
...
diff -u$u -L gen-compact-expr-cxxpy-depth-1-base0-array.old <(echo \
'$ print() { printf '\''%s\n'\'' "$@"; }
$ printc() { awk '\''{ printf("%c\n", strtonum("0x" $1)) }'\'' <<< "$1"; }
...
pot|t'
) -L gen-compact-expr-cxxpy-depth-1-base0-array.new <(
echo '$ print() { printf '\''%s\n'\'' "$@"; }'
print() { printf '%s\n' "$@"; } 2>&1 ||
...
echo '$ trie() { ../src/trie --gen=compact --output=expr --expr=cxxpy --depth=1 --trie=array "$@"; }'
trie() { ../src/trie --gen=compact --output=expr --expr=cxxpy --depth=1 --trie=array "$@"; } 2>&1 ||
echo 'command failed: trie() { ../src/trie --gen=compact --output=expr --expr=cxxpy --depth=1 --trie=array "$@"; }'
