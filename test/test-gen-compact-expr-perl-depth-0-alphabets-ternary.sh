...
# $ gen-test -T gen-compact-expr-perl-depth-0-alphabets-ternary
#
...
diff -u$u -L gen-compact-expr-perl-depth-0-alphabets-ternary.old <(echo \
'$ print() { printf '\''%s\n'\'' "$@"; }
$ printc() { awk '\''{ printf("%c\n", strtonum("0x" $1)) }'\'' <<< "$1"; }
...
) -L gen-compact-expr-perl-depth-0-alphabets-ternary.new <(
echo '$ print() { printf '\''%s\n'\'' "$@"; }'
print() { printf '%s\n' "$@"; } 2>&1 ||
...
echo '$ ../src/trie --dots --gen=compact --output=expr --expr=perl --depth=0 --trie=ternary < alphabets.txt'
../src/trie --dots --gen=compact --output=expr --expr=perl --depth=0 --trie=ternary < alphabets.txt 2>&1 ||
echo 'command failed: ../src/trie --dots --gen=compact --output=expr --expr=perl --depth=0 --trie=ternary < alphabets.txt'
)
