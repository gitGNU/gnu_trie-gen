...
# $ gen-test -T print-obj-base0-array
#
...
diff -u$u -L print-obj-base0-array.old <(echo \
'$ print() { printf '\''%s\n'\'' "$@"; }
$ printc() { awk '\''{ printf("%c\n", strtonum("0x" $1)) }'\'' <<< "$1"; }
...
echo '$ trie() { ../src/trie --node=str --trie=array "$@"; }'
trie() { ../src/trie --node=str --trie=array "$@"; } 2>&1 ||
echo 'command failed: trie() { ../src/trie --node=str --trie=array "$@"; }'
