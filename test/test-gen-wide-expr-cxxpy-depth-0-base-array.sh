...
# $ gen-test -T gen-wide-expr-cxxpy-depth-0-base-array
#
...
diff -u$u -L gen-wide-expr-cxxpy-depth-0-base-array.old <(echo \
'$ print() { printf '\''%s\n'\'' "$@"; }
$ printc() { awk '\''{ printf("%c\n", strtonum("0x" $1)) }'\'' <<< "$1"; }
...
) -L gen-wide-expr-cxxpy-depth-0-base-array.new <(
echo '$ print() { printf '\''%s\n'\'' "$@"; }'
print() { printf '%s\n' "$@"; } 2>&1 ||
...
echo '$ trie() { ../src/trie --gen=wide --output=expr --expr=cxxpy --depth=0 --dots --trie=array "$@"; }'
trie() { ../src/trie --gen=wide --output=expr --expr=cxxpy --depth=0 --dots --trie=array "$@"; } 2>&1 ||
echo 'command failed: trie() { ../src/trie --gen=wide --output=expr --expr=cxxpy --depth=0 --dots --trie=array "$@"; }'
