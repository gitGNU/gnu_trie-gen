...
# $ gen-test -T gen-compact-expr-cxxpy-depth-2-alphabets-array
#
...
diff -u$u -L gen-compact-expr-cxxpy-depth-2-alphabets-array.old <(echo \
'$ print() { printf '\''%s\n'\'' "$@"; }
$ printc() { awk '\''{ printf("%c\n", strtonum("0x" $1)) }'\'' <<< "$1"; }
...
) -L gen-compact-expr-cxxpy-depth-2-alphabets-array.new <(
echo '$ print() { printf '\''%s\n'\'' "$@"; }'
print() { printf '%s\n' "$@"; } 2>&1 ||
...
echo '$ ../src/trie --dots --gen=compact --output=expr --expr=cxxpy --depth=2 --trie=array < alphabets.txt'
../src/trie --dots --gen=compact --output=expr --expr=cxxpy --depth=2 --trie=array < alphabets.txt 2>&1 ||
echo 'command failed: ../src/trie --dots --gen=compact --output=expr --expr=cxxpy --depth=2 --trie=array < alphabets.txt'
)
