...
# $ gen-test -T gen-compact-c-code-filenames-array
#
...
diff -u$u -L gen-compact-c-code-filenames-array.old <(echo \
'$ print() { printf '\''%s\n'\'' "$@"; }
$ printc() { awk '\''{ printf("%c\n", strtonum("0x" $1)) }'\'' <<< "$1"; }
...
.   }
.   return error;'
) -L gen-compact-c-code-filenames-array.new <(
echo '$ print() { printf '\''%s\n'\'' "$@"; }'
print() { printf '%s\n' "$@"; } 2>&1 ||
...
echo '$ ../src/trie --attrs --dots --gen=compact --output=c-code --trie=array < filenames.txt'
../src/trie --attrs --dots --gen=compact --output=c-code --trie=array < filenames.txt 2>&1 ||
echo 'command failed: ../src/trie --attrs --dots --gen=compact --output=c-code --trie=array < filenames.txt'
)
