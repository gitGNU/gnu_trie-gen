...
# $ gen-test -T gen-wide-c-code-base0-ternary
#
...
diff -u$u -L gen-wide-c-code-base0-ternary.old <(echo \
'$ print() { printf '\''%s\n'\'' "$@"; }
$ printc() { awk '\''{ printf("%c\n", strtonum("0x" $1)) }'\'' <<< "$1"; }
...
$ print pot potato pottery tattoo tempo|trie
    ...
    return error;'
) -L gen-wide-c-code-base0-ternary.new <(
echo '$ print() { printf '\''%s\n'\'' "$@"; }'
print() { printf '%s\n' "$@"; } 2>&1 ||
...
echo '$ trie() { ../src/trie --gen=wide --output=c-code --trie=ternary "$@"; }'
trie() { ../src/trie --gen=wide --output=c-code --trie=ternary "$@"; } 2>&1 ||
echo 'command failed: trie() { ../src/trie --gen=wide --output=c-code --trie=ternary "$@"; }'
