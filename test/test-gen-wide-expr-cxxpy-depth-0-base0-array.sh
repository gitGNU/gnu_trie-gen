...
# $ gen-test -T gen-wide-expr-cxxpy-depth-0-base0-array
#
...
diff -u$u -L gen-wide-expr-cxxpy-depth-0-base0-array.old <(echo \
'$ print() { printf '\''%s\n'\'' "$@"; }
$ printc() { awk '\''{ printf("%c\n", strtonum("0x" $1)) }'\'' <<< "$1"; }
...
) -L gen-wide-expr-cxxpy-depth-0-base0-array.new <(
echo '$ print() { printf '\''%s\n'\'' "$@"; }'
print() { printf '%s\n' "$@"; } 2>&1 ||
...
echo '$ trie() { ../src/trie --gen=wide --output=expr --expr=cxxpy --depth=0 --trie=array "$@"; }'
trie() { ../src/trie --gen=wide --output=expr --expr=cxxpy --depth=0 --trie=array "$@"; } 2>&1 ||
echo 'command failed: trie() { ../src/trie --gen=wide --output=expr --expr=cxxpy --depth=0 --trie=array "$@"; }'
