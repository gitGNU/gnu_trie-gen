...
# $ gen-test -T gen-wide-expr-bash-depth-1-base
#
...
diff -u$u -L gen-wide-expr-bash-depth-1-base.old <(echo \
'$ print() { printf '\''%s\n'\'' "$@"; }
$ printc() { awk '\''{ printf("%c\n", strtonum("0x" $1)) }'\'' <<< "$1"; }
...
) -L gen-wide-expr-bash-depth-1-base.new <(
echo '$ print() { printf '\''%s\n'\'' "$@"; }'
print() { printf '%s\n' "$@"; } 2>&1 ||
...
echo '$ trie() { ../src/trie --gen=wide --output=expr --expr=bash --depth=1 --no-dots "$@"; }'
trie() { ../src/trie --gen=wide --output=expr --expr=bash --depth=1 --no-dots "$@"; } 2>&1 ||
echo 'command failed: trie() { ../src/trie --gen=wide --output=expr --expr=bash --depth=1 --no-dots "$@"; }'
