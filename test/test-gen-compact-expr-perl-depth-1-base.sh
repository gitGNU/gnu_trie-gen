...
# $ gen-test -T gen-compact-expr-perl-depth-1-base
#
...
diff -u$u -L gen-compact-expr-perl-depth-1-base.old <(echo \
'$ print() { printf '\''%s\n'\'' "$@"; }
$ printc() { awk '\''{ printf("%c\n", strtonum("0x" $1)) }'\'' <<< "$1"; }
...
a'
) -L gen-compact-expr-perl-depth-1-base.new <(
echo '$ print() { printf '\''%s\n'\'' "$@"; }'
print() { printf '%s\n' "$@"; } 2>&1 ||
...
echo '$ trie() { ../src/trie --gen=compact --output=expr --expr=perl --depth=1 --no-dots "$@"; }'
trie() { ../src/trie --gen=compact --output=expr --expr=perl --depth=1 --no-dots "$@"; } 2>&1 ||
echo 'command failed: trie() { ../src/trie --gen=compact --output=expr --expr=perl --depth=1 --no-dots "$@"; }'
