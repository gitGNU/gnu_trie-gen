...
# $ gen-test -T print-obj-filenames-array
#
...
diff -u$u -L print-obj-filenames-array.old <(echo \
'$ print() { printf '\''%s\n'\'' "$@"; }
$ printc() { awk '\''{ printf("%c\n", strtonum("0x" $1)) }'\'' <<< "$1"; }
...
) -L print-obj-filenames-array.new <(
echo '$ print() { printf '\''%s\n'\'' "$@"; }'
print() { printf '%s\n' "$@"; } 2>&1 ||
...
echo '$ ../src/trie --attrs --dots --output=object --trie=array < filenames.txt'
../src/trie --attrs --dots --output=object --trie=array < filenames.txt 2>&1 ||
echo 'command failed: ../src/trie --attrs --dots --output=object --trie=array < filenames.txt'
)
