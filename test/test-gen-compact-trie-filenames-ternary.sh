...
# $ gen-test -T gen-compact-trie-filenames-ternary
#
...
diff -u$u -L gen-compact-trie-filenames-ternary.old <(echo \
'$ print() { printf '\''%s\n'\'' "$@"; }
$ printc() { awk '\''{ printf("%c\n", strtonum("0x" $1)) }'\'' <<< "$1"; }
...
) -L gen-compact-trie-filenames-ternary.new <(
echo '$ print() { printf '\''%s\n'\'' "$@"; }'
print() { printf '%s\n' "$@"; } 2>&1 ||
...
echo '$ ../src/trie --attrs --dots --gen=compact --output=trie --trie=ternary < filenames.txt'
../src/trie --attrs --dots --gen=compact --output=trie --trie=ternary < filenames.txt 2>&1 ||
echo 'command failed: ../src/trie --attrs --dots --gen=compact --output=trie --trie=ternary < filenames.txt'
)
