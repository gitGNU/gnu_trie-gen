...
# $ gen-test -T gen-compact-expr-perl-depth-2-base-array
#
...
diff -u$u -L gen-compact-expr-perl-depth-2-base-array.old <(echo \
'$ print() { printf '\''%s\n'\'' "$@"; }
$ printc() { awk '\''{ printf("%c\n", strtonum("0x" $1)) }'\'' <<< "$1"; }
...
a(?:bcd)?|h(?:ijk|lm)?'
) -L gen-compact-expr-perl-depth-2-base-array.new <(
echo '$ print() { printf '\''%s\n'\'' "$@"; }'
print() { printf '%s\n' "$@"; } 2>&1 ||
...
echo '$ trie() { ../src/trie --gen=compact --output=expr --expr=perl --depth=2 --dots --trie=array "$@"; }'
trie() { ../src/trie --gen=compact --output=expr --expr=perl --depth=2 --dots --trie=array "$@"; } 2>&1 ||
echo 'command failed: trie() { ../src/trie --gen=compact --output=expr --expr=perl --depth=2 --dots --trie=array "$@"; }'
